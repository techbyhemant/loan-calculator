...
export default function BalanceTransferCalc() {
  ...
  const results = useMemo(() => {
    ...
    const oldEMI = oldR === 0 ? principal / months : (principal * oldR * Math.pow(1 + oldR, months)) / (Math.pow(1 + oldR, months) - 1);
    const newEMI = newR === 0 ? principal / months : (principal * newR * Math.pow(1 + newR, months)) / (Math.pow(1 + newR, months) - 1);
    ...
    const breakEvenMonths = monthlySaving > 0 ? Math.ceil(totalTransferCost / monthlySaving) : 0;
    const worthIt = netSaving > 25000;

    return { oldEMI, newEMI, oldTotalInterest, newTotalInterest, interestSaved, totalTransferCost, netSaving, monthlySaving, breakEvenMonths, worthIt };
  }, [outstanding, currentRate, newRate, remainingTenure, processingFee, legalCharges]);

  return (
    <div className="space-y-6">
      <CalcSection title="Enter Your Loan Details">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><Label>Outstanding Loan Amount (₹)</Label><NumericInput value={outstanding} onChange={setOutstanding} placeholder="40,00,000" min={0} className={CALC_INPUT_CLASS} /></div>
          <div><Label>Remaining Tenure (Months)</Label><NumericInput value={remainingTenure} onChange={setRemainingTenure} placeholder="180" min={1} max={360} className={CALC_INPUT_CLASS} /></div>
          <div><Label>Current Interest Rate (%)</Label><NumericInput value={currentRate} onChange={setCurrentRate} placeholder="9.0" min={0} max={20} step={0.1} className={CALC_INPUT_CLASS} /></div>
          <div><Label>New Bank Rate (%)</Label><NumericInput value={newRate} onChange={setNewRate} placeholder="8.5" min={0} max={20} step={0.1} className={CALC_INPUT_CLASS} /></div>
          <div><Label>Processing Fee (%)</Label><NumericInput value={processingFee} onChange={setProcessingFee} placeholder="0.5" min={0} max={3} step={0.1} className={CALC_INPUT_CLASS} /></div>
          <div><Label>Legal/Valuation Charges (₹)</Label><NumericInput value={legalCharges} onChange={setLegalCharges} placeholder="15,000" min={0} className={CALC_INPUT_CLASS} /></div>
        </div>
      </CalcSection>

      {results && (
        <div className="space-y-4">
          <Verdict type={results.worthIt ? "good" : results.netSaving > 0 ? "neutral" : "bad"}>
            {results.worthIt
            ...
            {results.breakEvenMonths > 0 && results.netSaving > 0 && (
              <span className="block text-sm font-normal mt-1">Break-even after {formatMonths(results.breakEvenMonths)}</span>
            )}
          </Verdict>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50">
                  ...
                  <th className="text-right px-4 py-3 font-medium">Current Bank</th>
                  <th className="text-right px-4 py-3 font-medium">New Bank</th>
                </tr>
              </thead>
