    ...
    <div className="space-y-6">
      ...
      {results && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <CalcCard className={results.recommendation === "PREPAY" ? "ring-2 ring-green-200" : ""}>
              <h2 className="text-base font-semibold mb-3">Prepay Your Loan</h2>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Interest Saved</span><span className="font-semibold text-green-700">{formatINR(results.prepay.interestSaved)}</span></div>
                ...
                <div className="border-t pt-2 flex justify-between font-semibold"><span>Net Benefit</span><span className="text-green-700">{formatINR(results.prepay.netBenefit)}</span></div>
              </div>
            </CalcCard>

            <CalcCard className={results.recommendation === "SIP" ? "ring-2 ring-blue-200" : ""}>
              <h2 className="text-base font-semibold mb-3">Invest in SIP</h2>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Corpus at Loan End</span><span className="font-semibold text-blue-700">{formatINR(results.sip.corpus)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">LTCG Tax</span><span className="font-semibold text-red-600">-{formatINR(results.sip.ltcgTax)}</span></div>
                <div className="border-t pt-2 flex justify-between font-semibold"><span>Net Corpus</span><span className="text-blue-700">{formatINR(results.sip.netCorpus)}</span></div>
              </div>
            </CalcCard>
          </div>
          ...
          <TableCard>
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                ...
                <tr><td className="px-4 py-3 text-muted-foreground">LTCG Tax</td><td className="px-4 py-3 text-right text-muted-foreground">—</td><td className="px-4 py-3 text-right text-red-600">-{formatINR(results.sip.ltcgTax)}</td></tr>
                <tr className="bg-muted/50 font-semibold"><td className="px-4 py-3">Net Benefit</td><td className="px-4 py-3 text-right text-green-700">{formatINR(results.prepay.netBenefit)}</td><td className="px-4 py-3 text-right text-blue-700">{formatINR(results.sip.netCorpus - results.sip.totalInvested)}</td></tr>
              </tbody>
            </table>
          </TableCard>
        </div>
